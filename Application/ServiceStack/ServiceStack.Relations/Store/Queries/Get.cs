...
namespace Demo.Application.ServiceStack.Relations.Store.Queries
{
    public class Get : IQueryHandler<Services.Get, Responses.Get>
    {
        ...
        public R.Query<Responses.Get> Handle(Services.Get query)
        {
            ...
            {
                var dto = session.Load<Responses.Get>(query.StoreId);

                var response = new R.Query<Responses.Get> { Payload = dto, Etag = Guid.Empty, SubscriptionId = query.SubscriptionId, SubscriptionTime = query.SubscriptionTime };
